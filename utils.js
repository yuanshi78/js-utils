 ...
 * 
 */
(function(Cls, exports){
	//Functions to work with objects
	// If there's no the function Object.create, define it
	if (typeof Object.create !== "function"){
		Object.create = function(obj){
			function EmptyContructor(){};
			EmptyContructor.prototype = obj;

			return new EmptyContructor(); 
		}
	}
	...
	Utils.extend({
		onDOMload: function(callback, context, args){
			var ieContentLoaded = function(callback, args){
				try {
					document.doScroll("left");
				} catch(e) {
					setTimeout(ieContentLoaded, 4);
				}

				Cls.proxy(callback, null, args)();
			}

			if (document.readyState != "complete"){
				if (!document.doScroll){
					if (document.addEventListener){
						document.addEventListener("DOMContentLoaded", Cls.proxy(callback, context, args));
					} else 
						document.attachEvent("onDOMContentLoaded", Cls.proxy(callback, context, args));
					}
				} else {
